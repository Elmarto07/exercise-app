...
/** Unique session dates — presence only (multiple sessions → one entry). */
export function indexStretchDates(
  stretchSessions: StretchSession[],
): Set<string> {
  return new Set(stretchSessions.map((session) => session.date));
}

export function dayHasStretchSession(
  date: string,
  stretchDates: Set<string>,
): boolean {
  return stretchDates.has(date);
}
...
export function buildHistoryListRows(
  ...
  return [...windowDates].reverse().map((date) => ({
    ...
    hasStretchSession: dayHasStretchSession(date, stretchDates),
  }));
}
